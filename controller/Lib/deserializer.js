...
Deserializer.prototype.deserialize = function(xml, callback) {
...
Deserializer.prototype.onDone = function() {
  ...
  if (!this.error) {
    if (this.type === null || this.marks.length) {
      ...
      this.callback(undefined, {methodName: this.methodname, params: this.stack})
    }
  }
}
...
Deserializer.prototype.onOpentag = function(node) {
  if (node.name.toUpperCase() === 'ARRAY' || node.name.toUpperCase() === 'STRUCT') {
